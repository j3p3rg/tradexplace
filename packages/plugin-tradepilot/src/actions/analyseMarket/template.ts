...
export const getOperationTemplate = `Respond with a JSON object containing ALWAYS symbol, currency, amount, and operation. Currency must default to "USD" if not specified.

Here are some cryptocurrency symbol mappings:
- bitcoin/btc -> BTC
- ethereum/eth -> ETH
- solana/sol -> SOL
- cardano/ada -> ADA
...
IMPORTANT: Response must ALWAYS include both "symbol", "currency", "amount", and "operation" fields.

Example #1 recentMessage:
Buy 0.02 ETH
...
Extract the cryptocurrency symbol, currency, amount, and operation from the most recent message. Always include currency (default "USD").
Respond with a JSON markdown block containing symbol, currency, amount, and operation.`;
...
- "advice": A string with one of the values "BUY", "SELL", or "WAIT".
...
If you are not sure about what advice to give then return "WAIT", and give an explanation mentioning that you cannot provide a clear advice and the reasons why.
